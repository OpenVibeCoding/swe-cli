...
class ConfigManager:
    ...
    def load_config(self) -> AppConfig:
        ...
        1. Local project config (.opencli/settings.json)
        2. Global user config (~/.opencli/settings.json)
        3. Default values
        """
        # Start with defaults
        config_data: dict = {}
        ...
        # Load global config
        global_config = Path.home() / ".opencli" / "settings.json"
        ...
        local_config = self.working_dir / ".opencli" / "settings.json"
    ...
    def save_config(self, config: AppConfig, global_config: bool = False) -> None:
        ...
        """
        if global_config:
            config_path = Path.home() / ".opencli" / "settings.json"
        else:
            config_path = self.working_dir / ".opencli" / "settings.json"
    ...
    def ensure_directories(self) -> None:
        ...
        # Expand paths
        opencli_dir = Path(config.opencli_dir).expanduser()
        session_dir = Path(config.session_dir).expanduser()
        log_dir = Path(config.log_dir).expanduser()

        # Create directories
        opencli_dir.mkdir(parents=True, exist_ok=True)
    ...
    def load_context_files(self) -> list[str]:
        ...
        # Global context
        global_context = Path.home() / ".opencli" / "OPENCLI.md"
