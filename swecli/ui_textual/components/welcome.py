...
class WelcomeMessage:
    ...
    def generate_full_welcome(
        ...
        right_entries: List[Tuple[str, str]] = [
            ...
            (f" • {CYAN}/clear{RESET}          Clear the conversation", "left"),
            (f" • {CYAN}/models{RESET}         Configure AI models", "left"),
            (f" • {CYAN}/mode [normal|plan]{RESET} Toggle execution mode", "left"),
            (f" • {CYAN}/init{RESET}           Analyze codebase structure", "left"),
            (f" • {CYAN}/quit{RESET}           Exit the application", "left"),
            ("", "left"),
            (f"{BOLD}MCP Integration{RESET}", "left"),
            (f" • {CYAN}/mcp [list|status|connect|tools]{RESET} Manage MCP servers", "left"),
            ("", "left"),
