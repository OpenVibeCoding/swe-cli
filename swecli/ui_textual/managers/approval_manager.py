...
class ChatApprovalManager:
    ...
    def request_approval_sync(
        ...
        *,
        ...
        """Synchronous version of request_approval for calling from background threads.

        This method handles cross-thread communication with the Textual UI properly,
        allowing approval to be requested from synchronous code running in background threads.

        Args:
            operation: Operation requiring approval
            preview: Preview text for the operation
            allow_edit: Whether to allow editing the command
            timeout: Timeout for approval (unused, kept for compatibility)
            command: Command string to approve
            working_dir: Working directory
            force_prompt: Force showing the prompt even if auto-approval would apply
        ...
        # Check auto-approval first (these don't need async)
        if not force_prompt:
            auto_result = self._check_auto_approval(operation, command)
            if auto_result:
                return auto_result
        ...
        # If no chat_app or not running, use fallback
        if not self.chat_app or not getattr(self.chat_app, "is_running", False):
            approved, choice, edited_command = self._fallback_prompt(
                command, working_dir, RuntimeError("Chat app unavailable")
            )
            if choice == "1":
                return self._process_approve_choice(command, edited_command, matched_rule)
            elif choice == "2":
        ...
        # Use concurrent.futures.Future for cross-thread synchronization
        result_future = concurrent.futures.Future()

        # Capture matched_rule for use in closures
        captured_matched_rule = matched_rule
        captured_command = command
        captured_working_dir = working_dir

        # Define the async approval coroutine
        async def do_approval():
            try:
                # Directly use the approval prompt controller (not _show_approval_modal)
                # This avoids the deadlock from nested call_from_thread
                controller = self.chat_app._approval_prompt_controller
                approved, choice, edited_command = await controller.start(
                    captured_command or "", captured_working_dir or ""
                )

                # Process user choice
                if choice == "1":
                    result = self._process_approve_choice(captured_command, edited_command, captured_matched_rule)
                elif choice == "2":
                    result = self._process_approve_all_choice(captured_command, edited_command, captured_matched_rule)
                else:
                    result = self._process_deny_choice(captured_command, captured_matched_rule)

                result_future.set_result(result)
            except Exception as e:
                result_future.set_exception(e)

        # Schedule the worker on the UI thread
        def schedule_approval():
            self.chat_app.run_worker(do_approval(), name="approval-sync", exclusive=True)

        self.chat_app.call_from_thread(schedule_approval)

        # Wait for result (blocking) - 5 minute timeout for user response
        try:
            return result_future.result(timeout=300)
            ...
            approved, choice, edited_command = self._fallback_prompt(command, working_dir, e)
            if choice == "1":
                return self._process_approve_choice(command, edited_command, matched_rule)
            elif choice == "2":
                return self._process_approve_all_choice(command, edited_command, matched_rule)
