"""Plan mode formatter for tool results."""

from typing import Any, Dict
from rich.panel import Panel
from rich.text import Text

from .formatter_base import BaseFormatter, TOOL_ICONS
from ..utils.tool_display import get_tool_display_parts, summarize_tool_arguments


class PlanFormatter(BaseFormatter):
    """Handles formatting for plan-only tool results."""
    ...
    def format_plan_result(
        ...
        if arguments:
            ...
            for key, value in arguments.items():
                body.append(
                    f"  • {key}: {self._summarize_value(value)}\n"
                )
        else:
            body.append("\n  • (no arguments)\n")
